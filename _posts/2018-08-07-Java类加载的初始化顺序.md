...
<p>1. 调用类的静态字段，只有直接定义该字段的类才会初始化，如通过子类调用父类的静态变量，只有父类会初始化</p>
<p>2. 静态常量不会触发初始化加载过程，static final 修饰的字段在 Javac 时生成 ConstantValue 属性，在类加载的准备阶段根据 ConstantValue 的值为该字段赋值，它没有默认值，必须显式地赋值，否则 Javac 时会报错。可以理解为在编译期即把结果放入了常量池中</p>
